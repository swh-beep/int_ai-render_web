...
def _analysis_catalog_context(item_data: dict | None, *, label: str, dims_mm: dict | None) -> str:
    ...
    lines = [
        ...
        f"Product name: {item_data.get('product_name') or 'unknown'}",
    ...
    try:
        with Image.open(moodboard_path) as img:
            prompt = (
                ...
                "**NOTE:** The background may be a furnished room or a neutral product-reference canvas. "
                "Do not detect the background or built-in architecture itself.\n"
...
def analyze_cropped_item(
    ...
    *,
    ...
    try:
        ...
        if not detailed_analysis_enabled and not ultra_detailed_analysis:
