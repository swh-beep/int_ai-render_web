...
def test_generate_detail_view_product_backed_detail_uses_locked_reframe_without_model_call(tmp_path):
    source_path = tmp_path / "room.png"
    source = Image.new("RGB", (1200, 1500), color=(245, 245, 245))
    for x in range(816, 1164):
        for y in range(150, 1290):
            source.putpixel((x, y), (32, 32, 32))
    source.save(source_path, format="PNG")

    def _call_gemini(model_name, content, request_options, safety_settings, **kwargs):
        raise AssertionError("product-backed detail must not regenerate target pixels")

    result = generate_detail_view(
        str(source_path),
        {
            "name": "Detail: Tolomeo Mega Floor",
            "target_key": "cart_product-2_tolomeo_002",
            "target_label": "Tolomeo Mega Floor",
            "detail_mode": "product_identity_lock",
            "ratio": "4:5",
            "target_box_2d": [100, 680, 860, 970],
            "prompt": "render an editorial lamp detail",
        },
        "unitcase",
        2,
        furniture_data=[
            {
                "label": "Tolomeo Mega Floor",
                "target_key": "cart_product-2_tolomeo_002",
                "category": "floor_lamp",
                "box_2d": [100, 680, 860, 970],
                "source_box_2d": [100, 680, 860, 970],
                "box_source": "main_render",
                "reference_features": {
                    "distinctive_parts": ["black drum shade", "articulated arm"],
                    "preserve_rules": ["shade stays black", "arm joints stay visible"],
                },
            }
        ],
        materialize_input=lambda path, prefix: path,
    ...
    output_path = Path(result["path"])
    try:
        assert result["generation_mode"] == "product_locked_reframe"
        assert result["product_pixel_lock"] is True
        assert result["locked_target_box_2d"] == [100.0, 680.0, 860.0, 970.0]
        assert result["crop_bounds_px"]
        assert output_path.exists()
        with Image.open(output_path) as rendered:
            assert rendered.size[1] > rendered.size[0]
...
def test_generate_detail_view_product_backed_detail_without_localized_box_does_not_fallback_to_model(tmp_path):
    source_path = tmp_path / "room.png"
    Image.new("RGB", (1200, 1500), color=(245, 245, 245)).save(source_path, format="PNG")
    ...
    result = generate_detail_view(
        str(source_path),
        {
            "name": "Detail: Layer Table Lamp",
            "target_key": "cart_product-3_layer_003",
            "target_label": "Layer Table Lamp",
            "detail_mode": "product_identity_lock",
            "ratio": "4:5",
            "target_box_2d": [0, 0, 1000, 1000],
            "prompt": "render an editorial lamp detail",
        },
        "unitcase",
        3,
        ...
        materialize_input=lambda path, prefix: (_ for _ in ()).throw(AssertionError("no cutout materialization")),
        normalize_label_for_match=lambda text: str(text or "").strip().lower(),
        allow_harassment_only_safety_settings=lambda: (_ for _ in ()).throw(AssertionError("no safety settings")),
        call_gemini_with_failover=lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("no model fallback")),
        model_name="gemini-3.1-flash-image",
    )
